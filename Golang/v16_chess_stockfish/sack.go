...
func (s *SACKEngine) addLink(nid, fieldPos int, before, after [CoarseCells]float32) {
	...
	if len(s.Chain) >= chainMax {
		...
		s.TotalChains++
		s.Chain = nil
		// Reset chain sig on flush — new chain, new journey
		s.ChainSig = 0
	}
}
...
func (s *SACKEngine) reinforceCheckGradient() {
	...
	for i, entry := range s.CheckHistory {
		reward := float64(i+1) / float64(l)
		boundaryAmp := 1.0
		if entry.Boundary {
			boundaryAmp = 1.4
		}
		coilWeight := 1.0 + float64(entry.Coil)/SpiralCoils*0.4
		strength := reward * boundaryAmp * coilWeight * 0.5
...
// reinforceCapture fires a low-weight bounce on the last chain link when a piece
// is captured mid-game. Weight is scaled by the captured piece's value so queen
// captures register more strongly than pawns. This builds trunk nodes in the
// causal chain — positions that are reused across games strengthen naturally.
// When combined with a simultaneous reinforceCheck call (capture+check), both
// signals stack on the same node, marking it as a high-value branch point.
...
func (d *DelegatorSack) reinforceCheck() {
	if child := d.activeChild(); child != nil {
		child.reinforceCheck()
	}
}

// reinforceCapture routes a mid-game capture bounce to the active piece child.
// Weight is pieceValue * 0.05 — small enough not to override outcome signals
// but sufficient to accumulate trunk chain density across hundreds of games.
func (d *DelegatorSack) reinforceCapture(capturedType string) {
	weight := pieceValue[capturedType] * 0.05
	if child := d.activeChild(); child != nil {
