...
func (srv *Server) doTick(sfPreFetch *ChessAPIResponse, sfPreFetchFEN string) TickResult {
	...
	if captured != nil {
		...
		// Mid-game capture reinforcement — builds trunk chain nodes.
		// Low weight (pieceValue * 0.05) so it doesn't override outcome signals.
		// If this capture also delivers check, reinforceCheck fires below and
		// stacks on the same node — marking it as a causal branch point.
		if !isStockfishTurn && captured.Type != "K" {
			sack.reinforceCapture(captured.Type)
		}
