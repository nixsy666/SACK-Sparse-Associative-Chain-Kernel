...
SACK.prototype.decide = function(angleDeg, distance, speed, pathConf) {
  ...
  // Safe tiebreak - fallback to sweep winner if no ties found
  var tied = unique.filter(function(nid) {
    return Math.abs(this.neurons[nid].resonate(fieldPos, speed) - best) < 0.01;
  }, this);
  if (tied.length > 0) bestNid = tied[Math.floor(Math.random() * tied.length)];

  this.lastConf = best;

  // Sliding window - keep last 10 decisions only
  this.lastChain.push({ nid: bestNid, fieldPos: fieldPos, pathConf: pathConf });
  if (this.lastChain.length > 10) this.lastChain.shift();
...
// STATS
// ============================================================================
function printStats(sack, run, score, pipesPassed, mode, pipeSpacing, allScores) {
  var s = sack.stats();
  var recent = allScores.slice(-8).join(" ");
  process.stdout.write("\x1B[2J\x1B[0f");
  console.log("====================================================");
  console.log("  SACK FLAPPY v5  |  Run " + run + "  |  Score " + score + "  |  " + mode + "  |  Spacing " + pipeSpacing);
  console.log("====================================================");
...
//    var useRandom = runNumber <= 2;
    var useRandom = false;
    var mode = useRandom ? "BOOTSTRAP" : "SACK";
    sack.resetChain();

    function tick() {
      if (!alive) {
        allScores.push(world.score);
        printStats(sack, runNumber, world.score, pipesPassed, mode, pipeSpacing, allScores);
      ...
      if (useRandom) {
        // Bootstrap: simple geometry - flap if bird is below gap centre
        var gapMid = world.pipes[0] ? world.pipes[0].gapTop + PIPE_GAP / 2 : WORLD_HEIGHT / 2;
        flap = world.birdY > gapMid;
      } else {
